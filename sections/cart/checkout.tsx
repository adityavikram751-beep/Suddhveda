...
function Stepper({ activeStep }: { activeStep: number }) {
  return (
    <div className="rounded-lg border border-[#F4D7B8] bg-white/55 px-1.5 sm:px-4 py-2 sm:py-4 shadow-sm mb-4 sm:mb-8 w-full max-w-full overflow-hidden">
...
const DeliveryAddressForm = forwardRef<HTMLDivElement, DeliveryAddressFormProps>(
  (
    ...
    return (
      <div ref={ref} className="rounded-[16px] border border-[#F2EFE9] bg-white p-3.5 sm:p-7 w-full max-w-full">
        <div className="flex items-center justify-between gap-2">
          <h2 className="font-serif text-[16px] sm:text-[19px] font-bold truncate">
        ...
        <div className="mt-4 sm:mt-6 grid gap-3.5 sm:gap-5 sm:grid-cols-2">
          <FormField
            label="Full Name"
            required
        ...
        <div className="mt-5 sm:mt-6 flex justify-center sm:justify-end">
          <button
            type="button"
            onClick={onSave}
            className="flex h-[42px] px-6 w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-[#F24E1E] hover:bg-[#D93F13] text-[12.5px] font-bold text-white shadow-md hover:shadow-lg hover:shadow-[#F24E1E]/35 hover:-translate-y-1 transition-all duration-300 cursor-pointer active:translate-y-0 active:scale-95 whitespace-nowrap"
          >
            {buttonLabel}
            <ArrowRight size={16} />
          </button>
      ...
      <div className="flex gap-2 min-w-0 w-full">
        {as === "select" ? (
          <select
            ...
            value={value}
            onChange={onChange}
            className="h-10 sm:h-11 w-full min-w-0 rounded-lg border border-[#E3E6EB] bg-white px-3 text-[12px] sm:text-[13px] text-[#2F241C] outline-none focus:border-[#593102]"
          >
          ...
          <input
            ...
            className="h-10 sm:h-11 w-full min-w-0 rounded-lg border border-[#E3E6EB] px-3 text-[12px] sm:text-[13px] text-[#2F241C] outline-none placeholder:text-[#B5BBC5] focus:border-[#593102]"
          />
        )}
        {action && (
          <button
            ...
            className={`flex h-10 sm:h-11 shrink-0 items-center gap-1.5 rounded-lg px-3 sm:px-5 text-[12px] sm:text-[13px] font-bold text-white transition-colors ${action.done ? "bg-[#0BA445]" : "bg-[#D18500] hover:bg-[#B97100]"
              }`}
          >
...
function SavedAddresses({ addresses, selectedId, onSelect, onAddNew, onEdit, onDelete }: any) {
  ...
  return (
    <div className="rounded-[16px] border border-[#F2EFE9] bg-white p-3.5 sm:p-7">
      <div className="flex items-center justify-between gap-2">
        <h2 className="font-serif text-[15px] sm:text-[19px] font-bold truncate">Select Saved Address</h2>
        <button
          type="button"
          onClick={onAddNew}
          className="text-[12px] sm:text-[13px] font-semibold text-[#593102] hover:underline shrink-0"
        >
          + Add New
        </button>
      </div>
