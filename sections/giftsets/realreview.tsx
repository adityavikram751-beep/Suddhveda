...
function StarRating({ rating }: { rating: number }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {Array.from({ length: fullStars }).map((_, i) => (
        <Star key={i} size={20} className="fill-[#A87400] text-[#A87400]" />
      ))}
      {hasHalf && (
        <StarHalf size={20} className="fill-[#A87400] text-[#A87400]" />
      )}
    </div>
  );
}
      ...
      {!loading && reviews.length > 0 && (
        <section className="relative bg-white py-12 sm:py-16 md:py-20">
          <div className="w-full max-w-[1300px] mx-auto px-4 sm:px-6">
            ...
            <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
              {/* Mobile view ke liye padding badha di hai taaki side cards touch ya overlap na karein */}
              <div ref={containerRef} className="px-3 sm:px-0">
                <div
                  className="flex transition-transform duration-700 ease-in-out"
                  style={{ transform: `translateX(-${currentIndex * 100}%)` }}
                >
                  {pages.map((pageItems, pageIdx) => (
                    <div
                      key={pageIdx}
                      className="w-full flex-shrink-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 px-1"
                      style={{
                        gridTemplateColumns: `repeat(${visibleCount}, minmax(0, 1fr))`,
                      }}
                    >
                      {pageItems.map((review) => (
                        <div
                          key={review._id}
                          className="w-full h-[320px] sm:h-[340px] bg-white border border-[#EFE7DF] rounded-[32px] p-6 sm:p-7 shadow-[0_10px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_18px_45px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
                        >
                          <div>
                            <div className="mb-4">
                              <StarRating rating={review.rating || 5} />
                          ...
                          <div className="flex items-center gap-3.5 pt-4 border-t border-[#F7F2EC]">
                            <div className="relative w-11 h-11 sm:w-12 sm:h-12 rounded-full overflow-hidden shrink-0 border border-[#EFE7DF]">
                              <Image
                                src={review.image || "/female.png"}
                                alt={review.name}
                                fill
                            ...
                            <div className="min-w-0">
                              <h4 className="text-[15px] sm:text-[16px] font-bold text-[#2D2016] leading-tight truncate">
                                {review.name}
                              </h4>
                              <p className="mt-0.5 text-[11px] uppercase tracking-[0.12em] font-medium text-[#8A6A52]">
                                {review.role || "Verified Buyer"}
                              </p>
                            </div>
                          </div>
